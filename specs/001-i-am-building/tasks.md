...
- [ ] T006 [P] Provision shared fixtures in [`backend/tests/conftest.py`](backend/tests/conftest.py) for DB, IMAP, Ollama, Qdrant, Gmail stubs — Supports TDD workflow — Depends on: T001
...
- [ ] T007 [P] Add JSON schema v2 contract suite in [`backend/tests/contract/test_schema_v2.py`](backend/tests/contract/test_schema_v2.py) using `classification_schema_v2.json` fixtures — Enforces deterministic classifier output — Depends on: T006
